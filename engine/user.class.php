...
class user {
    ...
    public function __construct($core) {
        ...
        $query = $this->db->query("SELECT `u`.`{$us_f['group']}`, `u`.`{$us_f['login']}`, `u`.`{$us_f['email']}`, `u`.`{$us_f['pass']}`, `u`.`{$us_f['salt']}`,
											...
											`u`.`{$us_f['is_skin']}`, `u`.`{$us_f['is_cloak']}`, `u`.`{$us_f['color']}`, `u`.`{$us_f['uuid']}`, `u`.`{$us_f['realmoney']}`,
        ...
        $this->realmoney	= intval($ar[$us_f['realmoney']]);

        if ($this->time_last + $core->cfg->func['online_time'] < time()) $core->logger->updateLastAction($this);
    }
