...
class submodule{
	...
	private function news_array() {
		$start		= $this->core->pagination($this->cfg->pagin['adm_news'], 0, 0);
		$end		= $this->cfg->pagin['adm_news']; 
		...
		if(!$query || $this->db->num_rows($query) <= 0 ) { 
			return Theme::render("modules/admin/news/new-none"); 
		}

		ob_start();
		while($ar = $this->db->fetch_assoc($query)){
			echo Theme::render("modules/admin/news/new-id", [
				"{id}" => intval($ar['id']),
				"{cid}" => intval($ar['cid']),
	...
	private function news_list() {
		...
		return Theme::render("modules/admin/news/new-list", [
			'{pagination}' => $this->core->pagination($this->cfg->pagin['adm_news'], $page.'&pid=', intval($ar[0])), 
	...
	private function delete() {
		if(!$this->core->is_access('sys_adm_news_delete')) { 
			$this->core->notify($this->core->lng["e_msg"], $this->core->lng['e_403'], 2, '?mode=admin&do=news'); 
		}

		if($_SERVER['REQUEST_METHOD'] != 'POST') { 
			$this->core->notify($this->core->lng["e_msg"], $this->core->lng['e_hack'], 2, '?mode=admin&do=news'); 
		}
			
		$list = @$_POST['id'];
		if(empty($list)) { 
			$this->core->notify($this->core->lng["e_msg"], $this->lng['news_not_selected'], 2, '?mode=admin&do=news'); 
		}
		...
		if(!$this->db->remove_fast("mcr_news", "id IN ($list)")) { 
			$this->core->notify($this->core->lng["e_msg"], $this->core->lng["e_sql_critical"], 2, '?mode=admin&do=news'); 
		}
	...
	private function categories($selected = 1) {
		...
		if(!$query || $this->db->num_rows($query) <= 0) {
			return Theme::render("modules/admin/news/cid-list-id", [
				"{id}" => 1,
				"{name}" => $this->lng['news_wo_cats'],
		...
		ob_start();
		while($ar = $this->db->fetch_assoc($query)) {
			echo Theme::render("modules/admin/news/cid-list-id", [
				"{id}" => intval($ar['id']),
				"{name}" => $this->db->HSC($ar['title']),
